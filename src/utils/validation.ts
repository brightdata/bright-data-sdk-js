...
export function validateUrl(url: unknown) {
    ...
    try {
        ...
        if (!parsed.protocol)
            throw new ValidationError(
                'URL must include a scheme (http/https)' + `: ${url}`,
            );
        ...
        if (!parsed.hostname)
            throw new ValidationError(
                'URL must include a valid domain: ' + `${url}`,
            );
        if (/[<>"']/.test(url))
...
export function validateZoneName(zone: string | void) {
    if (!zone) return;
    if (typeof zone != 'string') {
        throw new ValidationError(
            'Zone name must be a string, got ' + `${typeof zone}`,
        );
    }
    ...
    if (trimmed_zone.startsWith('-') || trimmed_zone.endsWith('-'))
        throw new ValidationError(
            'Zone name cannot start or end with a ' + 'hyphen',
        );
    if (trimmed_zone.startsWith('_') || trimmed_zone.endsWith('_'))
        throw new ValidationError(
            'Zone name cannot start or end with an ' + 'underscore',
        );
}

export function validateCountryCode(country: unknown) {
    if (typeof country != 'string') {
        throw new ValidationError(
            'Country code must be a string, got ' + `${typeof country}`,
        );
    }
    const trimmed_country = country.trim().toLowerCase();
    if (trimmed_country.length == 0) return;
...
export function validateTimeout(timeout: unknown) {
    if (timeout == null) return;
    if (typeof timeout !== 'number' || !Number.isInteger(timeout)) {
        throw new ValidationError(
            'Timeout must be an integer, got ' + `${typeof timeout}`,
        );
    }
    if (timeout <= 0) {
        throw new ValidationError(
    ...
    if (timeout > 300 * 1000) {
        throw new ValidationError(
            'Timeout cannot exceed 300000 milliseconds ' + '(5 minutes)',
        );
    }
}

export function validateMaxWorkers(max_workers: unknown) {
    if (max_workers == null || max_workers == undefined) return;
    if (typeof max_workers !== 'number' || !Number.isInteger(max_workers)) {
        throw new ValidationError(
            'max_workers must be an integer, got ' + `${typeof max_workers}`,
        );
    }
    if (max_workers <= 0)
    ...
    if (max_workers > 50)
        throw new ValidationError(
            'max_workers cannot exceed 50 (to prevent ' +
                'resource exhaustion)',
        );
}

export function validateUrlList(urls: unknown, max_urls = 100) {
    if (!Array.isArray(urls))
        throw new ValidationError(
            'URL list must be an array, got ' + `${typeof urls}`,
        );
    if (urls.length == 0) throw new ValidationError('URL list cannot be empty');
    if (urls.length > max_urls)
        throw new ValidationError(
            'URL list cannot contain more than ' + `${max_urls} URLs`,
        );
    urls.forEach((url, i) => {
        try {
            validateUrl(url);
            ...
            throw new ValidationError(
                `Invalid URL at index ${i}: ` + `${e.message}`,
            );
        }
    });
}

export function validateSearchEngine(search_engine: unknown) {
    if (typeof search_engine != 'string') {
        throw new ValidationError(
            'Search engine must be a string, got ' + `${typeof search_engine}`,
        );
    }
    ...
    if (!valid_engines.includes(normalized_engine))
        throw new ValidationError(
            `Invalid search engine '${search_engine}'. ` +
...
export function validateQuery(query: unknown) {
    if (typeof query == 'string') {
        ...
        if (query.length > 2048)
            throw new ValidationError(
                'Search query cannot exceed 2048 ' + 'characters',
            );
        ...
        if (query.length > 50)
            throw new ValidationError(
                'Query list cannot contain more than ' + '50 queries',
            );
        query.forEach((q, i) => {
            if (typeof q != 'string')
                throw new ValidationError(
                    `Query at index ${i} must be a ` +
                        `string, got ${typeof q}`,
                );
            if (!q.trim())
                throw new ValidationError(
                    `Query at index ${i} cannot be ` + 'empty or whitespace',
                );
            if (q.length > 2048)
                throw new ValidationError(
                    `Query at index ${i} cannot exceed ` + '2048 characters',
                );
        });
    } else
        throw new ValidationError(
            'Query must be a string or array of ' +
                `strings, got ${typeof query}`,
        );
}

export function validateResponseFormat(response_format: unknown) {
    if (typeof response_format != 'string')
        throw new ValidationError(
            'Response format must be a string, got ' +
...
export function validateHttpMethod(method: unknown) {
    if (typeof method != 'string')
        throw new ValidationError(
            'HTTP method must be a string, got ' + `${typeof method}`,
        );
    const valid_methods = ['GET', 'POST', 'PUT', 'DELETE', 'PATCH'];
    const normalized_method = method.trim().toUpperCase();
    if (!valid_methods.includes(normalized_method))
        throw new ValidationError(
            `Invalid HTTP method '${method}'. Valid ` +
